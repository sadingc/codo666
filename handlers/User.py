...
async def config(ev,bot,jdb,message_edited=None):
    ...
    buttons = []
    buttons.append([Button.inline('🏠Volver','user_start')])
...
async def start(ev,bot,jdb,message_edited=None):
    text = '👋 Bienvenido A CodiGod bot.👋\n\n'
    text+= 'Toma Nota....✍🏼\n\n'
    text+= '🧑🏻‍🏫 CodiGod bot es un bot simple,facil y rapido, destinado a la descarga y subida de contenido gratis en cuba 🙀. Si asi como oyes.!\n\n'
    text+= 'Su uso es sencillo , aca de bajo 👇 te dejo los ajustes.\n'

    buttons = [
        [Button.inline('⚙️Ajustes⚙️','user_config')]
    ]
